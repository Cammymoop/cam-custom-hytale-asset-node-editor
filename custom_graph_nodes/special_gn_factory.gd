...
func make_special_gn(root_asset_node: HyAssetNode, target_asset_node: HyAssetNode) -> CustomGraphNode:
    ...
    return call("make_special_%s" % target_asset_node.an_type, root_asset_node, target_asset_node) as CustomGraphNode

func make_special_ManualCurve(_root_asset_node: HyAssetNode, target_asset_node: HyAssetNode) -> CustomGraphNode:
    var new_manual_curve_gn: = ManualCurveSpecialGN.new()
    ...
    new_manual_curve_gn.load_points_from_connections()
