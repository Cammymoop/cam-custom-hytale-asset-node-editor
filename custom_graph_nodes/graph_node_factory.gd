...
func make_new_graph_node_for_asset_node(asset_node: HyAssetNode, is_newly_created: bool, at_position: Vector2, centered: bool = false) -> CustomGraphNode:
    ...
    if not is_special:
        setup_common_graph_node_controls(graph_node, asset_node)
...
func setup_common_graph_node_controls(graph_node: CustomGraphNode, asset_node: HyAssetNode) -> void:
    var node_schema: = graph_node.node_type_schema
    
    # NOTE: dumb hack
    get_tree().current_scene.add_child(graph_node)
    ...
    if node_schema and node_schema.get("no_output", false):
        num_outputs = 0
    graph_node.num_outputs = num_outputs
    ...
    var num_inputs: = connection_names.size()
    graph_node.num_inputs = num_inputs
    ...
    var first_setting_slot: = maxi(num_outputs, num_inputs)
    var total_slots: = first_setting_slot + num_settings
    ...
    for i in range(first_setting_slot, total_slots):
    ...
    # NOTE: dumb hack
    get_tree().current_scene.remove_child(graph_node)
